...
export class NavigationService {
    ...
    try {
      if (coordinates.length === 1) {
        ...
        if (
          ...
          response.data.waypoints.length === 0
        ) {
   ...
   */
  private async getTraficParTroncon(coordinates: number[][]): Promise<
    Array<{
      start: number[];
      end: number[];
  ...
  async getBasicRoute(query: GetRouteDto): Promise<any> {
    ...
    try {
      this.logger.log(
        `Appel OSRM (Cameroun, mode: ${query.mode || 'driving'}): ${url}`,
      );
      const response: AxiosResponse<any> = await firstValueFrom(
        this.httpService.get(url),
      );
  ...
  async getSmartRoute(query: GetRouteDto): Promise<any> {
    ...
    try {
      ...
      for (
        let routeIndex = 0;
        routeIndex < osrmResponse.data.routes.length;
        routeIndex++
      ) {
        ...
        for (const inc of activeIncidents) {
          ...
          const isNear = routeCoords.some((coord) => {
            ...
            const isNearStartOrEnd =
              distanceEnMetres(coord[1], coord[0], startLat, startLng) <= 30 ||
        ...
        const degradedCount =
          intersectedDegradedIncidents.size + (hasDegradedShock ? 1 : 0);
        const trafficCount = intersectedTrafficIncidents.size;

        // Calcul du score de confort (0 à 100)
        let comfortScore =
          100 - floodCount * 40 - degradedCount * 20 - trafficCount * 10;
        ...
        if (comfortScore >= 80) {
          ...
          comfortLevel = 'moyen';
          recommendation =
            'Route dégradée détectée, ralentissez et soyez vigilant.';
        } else {
          comfortLevel = 'mauvais';
          recommendation =
            'Route très dégradée ou inondation signalée, prudence extrême ou contournement conseillé !';
        }

        // Durée virtuelle pénalisée : +30 min par inondation, +10 min par dégradation, +5 min par trafic
        const totalPenalty =
          floodCount * 1800 + degradedCount * 600 + trafficCount * 300;
      ...
      if (routingMode === 'confort') {
        ...
        if (evaluatedRoutes[0].degradedCount > 0) {
          ...
          const alternativeWithoutTravaux = evaluatedRoutes.find(
            (r) => r.degradedCount === 0,
          );
          if (
            alternativeWithoutTravaux &&
            alternativeWithoutTravaux !== evaluatedRoutes[0]
          ) {
            const absoluteFastest = evaluatedRoutes[0];
            // Si le détour ne rallonge pas la durée de plus de 30%
            if (
              alternativeWithoutTravaux.route.duration <=
              absoluteFastest.route.duration * 1.3
            ) {
      ...
      if (
        routingMode === 'confort' &&
        (bestChoice.hasFlood || bestChoice.hasDegraded) &&
        ...
        try {
            ...
            activeIncidents.find(
              (inc) =>
                ...
                bestChoice.route.geometry.coordinates.some((coord) => {
                  const isNearStartOrEnd =
                    distanceEnMetres(coord[1], coord[0], startLat, startLng) <=
                      30 ||
                    distanceEnMetres(coord[1], coord[0], endLat, endLng) <= 30;
                  if (isNearStartOrEnd) return false;
                  return (
          ...
          if (criticalIncident) {
            this.logger.log(
              `[CONTOURNEMENT] Incident critique identifié sur le tracé : ${criticalIncident.type} à [${criticalIncident.latitude}, ${criticalIncident.longitude}]`,
            );
            ...
            if (
              nearestResponse.data.code === 'Ok' &&
              nearestResponse.data.waypoints
            ) {
              // Filtrer pour obtenir des points situés à plus de 55m (autre rue) mais moins de 350m (pas trop loin)
              const candidates = nearestResponse.data.waypoints.filter(
                (wp: any) => wp.distance >= 55 && wp.distance <= 350,
              );

              this.logger.log(
                `[CONTOURNEMENT] Nombre de segments de rue alternatifs candidats trouvés : ${candidates.length}`,
              );
              ...
              for (let cIdx = 0; cIdx < candidatesToTry.length; cIdx++) {
                ...
                this.logger.log(
                  `[CONTOURNEMENT] Test du candidat de contournement #${cIdx + 1} à [${perpLat}, ${perpLng}] (distance de l'incident : ${Math.round(candidate.distance)}m)`,
                );
                ...
                try {
                  const bypassResponse: AxiosResponse<any> =
                    await firstValueFrom(this.httpService.get(bypassUrl));

                  if (
                    bypassResponse.data.code === 'Ok' &&
                    bypassResponse.data.routes.length > 0
                  ) {
                    ...
                    for (const inc of activeIncidents) {
                      ...
                      const isNear = bCoords.some((c) => {
                        const isNearStartOrEnd =
                          distanceEnMetres(c[1], c[0], startLat, startLng) <=
                            30 ||
                      ...
                      if (isNear) {
                        if (inc.type === 'INONDATION')
                          bFloodIncidents.add(inc.id);
                        else if (
                          inc.type === 'QUALITE_ROUTE' ||
                          inc.type === 'ROUTE_ENDOMMAGEE'
                        )
                          bDegradedIncidents.add(inc.id);
                      }
                    }
                    ...
                    const bComfort =
                      100 - bFloodCount * 40 - bDegradedCount * 20;
                    ...
                    this.logger.log(
                      `[CONTOURNEMENT] Candidat #${cIdx + 1} évalué : scoreConfort=${evalResult.comfortScore} (incidents : ${bFloodCount} inon, ${bDegradedCount} deg), durée réelle : ${Math.round(bRoute.duration)}s`,
                    );

                    // Le contournement n'est conservé que s'il ne dépasse pas la durée originale du trajet + 5 minutes
                    if (
                      bRoute.duration <=
                      bestChoice.route.duration * 2.5 + 300
                    ) {
                      if (
                        bestBypassEvaluation === null ||
                        evalResult.comfortScore >
                          bestBypassEvaluation.comfortScore ||
                        (evalResult.comfortScore ===
                          bestBypassEvaluation.comfortScore &&
                          evalResult.penalizedDuration <
                            bestBypassEvaluation.penalizedDuration)
                      ) {
                        bestBypassEvaluation = evalResult;
                      }
                    } else {
                      this.logger.log(
                        `[CONTOURNEMENT] Candidat #${cIdx + 1} rejeté car trop long (${Math.round(bRoute.duration)}s contre original ${Math.round(bestChoice.route.duration)}s avec limite)`,
                      );
                    }
                  }
                } catch (e) {
                  this.logger.warn(
                    `Échec du calcul de déviation via le candidat #${cIdx + 1}: ${e.message}`,
                  );
                }
              }

              if (
                bestBypassEvaluation &&
                bestBypassEvaluation.comfortScore > bestChoice.comfortScore
              ) {
                bestChoice = bestBypassEvaluation;
                this.logger.log(
                  `[CONTOURNEMENT] Rerouting appliqué avec succès ! Confort amélioré : ${bestChoice.comfortScore}/100`,
                );
              } else {
                this.logger.log(
                  `[CONTOURNEMENT] Aucun itinéraire alternatif n'a pu améliorer le confort sans dépasser la limite de temps.`,
                );
              }
            }
          }
      ...
      if (selectedRouteIndex === -1) {
        ...
        this.logger.log(
          `[INSTRUCTIONS] Extraction des instructions depuis la route de contournement`,
        );
      } else {
      ...
      for (const incident of activeIncidents) {
        ...
        const isNearRoute = selectedRouteCoords.some(
          (coord) =>
            distanceEnMetres(
              incident.latitude!,
              incident.longitude!,
